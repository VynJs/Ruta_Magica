...
def admin_establecimientos_view(page: ft.Page) -> ft.View:

    usuario = page.app_state.get("usuario")
    nombre_usuario = getattr(usuario, "nombre", "Administrador") if usuario else "Administrador"

    panel_detalle = ft.Column([ft.Text("Selecciona un establecimiento de la lista para ver sus detalles.", color=MUTED)])

    def ir_a_editar(id_est):
        page.app_state["establecimiento_sel"] = id_est
        page.go("/admin/establecimientos/editar")

    def ir_a_ver(id_est):
        page.app_state["establecimiento_sel"] = id_est
        page.go("/admin/establecimientos/ver")

    def mostrar_detalle(est):
        ...
        panel_detalle.controls.extend([
            ...
            ft.Text(est.nombre_establecimiento, color=TEXT, size=18, weight=ft.FontWeight.BOLD),
            ft.Container(content=ft.Text(str(est.estado), color=BG, size=11), bgcolor=BTN_GREEN,
                         border_radius=12, padding=ft.padding.symmetric(horizontal=10, vertical=3)),
            ft.Text(f"ID: {est.id}", color=MUTED, size=11),
            ft.Divider(color=BORDER),
            ft.Text("Información", color=GOLD, weight=ft.FontWeight.BOLD),
            ft.Row([ft.Text("Categoría:", color=TEXT), ft.Text(str(est.categoria), color=MUTED)]),
            ft.Row([ft.Text("Dirección:", color=TEXT), ft.Text(str(est.direccion), color=MUTED)]),
            ft.Row([ft.Text("Teléfono:", color=TEXT), ft.Text(str(est.telefono), color=MUTED)]),
            ft.Row([ft.Text("Horario:", color=TEXT), ft.Text(f"{est.horario_inicio} - {est.horario_fin}", color=MUTED)]),
            ft.Container(height=10),
            ft.Row([
                ft.ElevatedButton("Editar establecimiento", icon=ft.icons.EDIT_OUTLINED,
                                  style=ft.ButtonStyle(bgcolor=GOLD, color=BG), on_click=lambda e: ir_a_editar(est.id)),
                ft.OutlinedButton("Enviar mensaje", icon=ft.icons.MAIL_OUTLINE, style=ft.ButtonStyle(color=TEXT),
                                  on_click=lambda e: ir_a_ver(est.id)),
            ]),
        ])
        page.update()
    ...
    def _fila(est) -> ft.Container:
        estado_txt = str(est.estado).lower()
        color_estado = BTN_GREEN if estado_txt in ("aprobado", "activo") else (ROJO if estado_txt in ("rechazado", "rechazada", "inactivo") else GOLD)
        return ft.Container(
            content=ft.Row(
                [
                    ft.Row([ft.Container(bgcolor="#DDDDDD", width=35, height=35, border_radius=6),
                            ft.Text(est.nombre_establecimiento, color=TEXT, width=140)]),
                    ft.Text(str(getattr(est, "nombre_propietario", "—")), color=MUTED, width=120),
                    ft.Text(str(est.categoria), color=TEXT, width=110),
                    ft.Container(content=ft.Text(str(est.estado), color=BG, size=11), bgcolor=color_estado,
    ...
    def eliminar(id_est):
        try:
            EstablecimientoDAO().eliminar(id_est)
            cargar_lista()
    ...
    def cargar_lista():
        ...
        try:
            establecimientos = EstablecimientoDAO().obtener_todo()
            contador["total"] = len(establecimientos)
            for est in establecimientos:
                estado_txt = str(est.estado).lower()
                if estado_txt in ("aprobado", "activo"):
                    contador["aprobados"] += 1
                elif estado_txt in ("en revision", "en revisión"):
                    contador["revision"] += 1
                elif estado_txt in ("rechazado", "rechazada"):
                    contador["rechazados"] += 1
                tabla_filas.controls.append(_fila(est))
    ...
    encabezado = ft.Row(
        [
            ft.Column([ft.Text("Establecimientos", color=GOLD, size=26, weight=ft.FontWeight.BOLD),
                       ft.Text("Administra, edita, actualiza y agrega nuevos establecimientos.", color=MUTED)], spacing=0),
            ft.Row([
    ...
    tarjetas = ft.Row(
        [
            _tarjeta_stat(ft.icons.STOREFRONT_OUTLINED, "Total establecimientos", contador["total"], "Registrados en la plataforma"),
    ...
    buscador = ft.TextField(hint_text="Buscar establecimiento...", prefix_icon=ft.icons.SEARCH,
                             bgcolor=BG, border_color=BORDER, color=TEXT, border_radius=8, width=260)

    lista_container = ft.Container(
        content=ft.Column(
            [
                ft.Text("Lista de establecimientos", color=TEXT, size=16, weight=ft.FontWeight.BOLD),
    ...
    return ft.View(
        route="/admin/establecimientos", bgcolor=BG, padding=0,
        controls=[ft.Row([_sidebar(page, "gestion_establecimientos"), ft.Container(content=contenido, padding=25, expand=True)],
